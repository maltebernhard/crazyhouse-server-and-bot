...
public class CrazyhouseGame extends Game implements Serializable{
	...
	public boolean tryMove(String moveString, Player player) {
		...
		if (this.nextPlayer == player && board.tryMove(moveString,nextPlayerString())) {	// richtiger Spieler am Zug && Zugdurchf�hrung erfolgrich
			history.add(new Move(moveString,state,player));										// Update history
			if (player==blackPlayer) {															// Unterscheidung: hat Schwarz oder Wei� gezogen?
				if (board.isGameOver("w")) {														// Spielzustand updaten falls Schachmatt
					blackPlayer.setWinner();
					finished=true;
				}
				nextPlayer=whitePlayer;																// n�chsten Spieler setzem
			}
			else {
